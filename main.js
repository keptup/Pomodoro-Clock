...
const toReadableConverter = (ms) => {
  ...
  let seconds = parseInt((ms / 1000 * 1) % 60);
	...
	const timeFormat = hours > 0 ? `${hours}:${minutes}:${seconds}` : `${minutes}:${seconds}`;
	return timeFormat;
  };
const countDownClock = (setTimeParam, startButtonClickTimeParam) => {
  difference = stopButtonClickTime === 0 ? setTimeParam + startButtonClickTimeParam - new Date().getTime() : setTimeParam + startButtonClickTimeParam - new Date().getTime();
